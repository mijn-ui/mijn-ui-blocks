...
import { KanbanProvider, KanbanProviderProps, useKanban } from "./context";
...
const KanbanDraggable = ({
  ...
  return (
    <div
      ...
      <button
        {...listeners}
        className="absolute right-4 top-4 flex size-6 items-center justify-center rounded-md border border-border transition-opacity sm:opacity-0 sm:group-hover:opacity-100">
...
>(({ className, ...props }, ref) => (
  <div
    className={cn(
      "w-full cursor-pointer space-y-4 rounded-lg bg-background p-4",
      className
    )}
    ref={ref}
    {...props}
